"""Command-line interface for NTSB API."""
...
def download(year, month, mode, output, api_url, extract_json):
...
def download_range(start_date, end_date, mode, output, api_url, extract_json):
...
@click.option("--api-url", default="http://localhost:8000", help="API server URL")
def cases(start_date, end_date, mode, limit, api_url):
    ...
    try:
        ...
        for case in result["data"]:
            click.echo(f"- {case.get('EventId', 'N/A')}: {case.get('EventDate', 'N/A')}")
    except Exception as e:
        click.echo(f"✗ Error: {e}", err=True)
        raise click.Abort()
